...
proc bMotion_plugins_irc_default_quit { nick host handle channel text } {
	...
	if [regexp -nocase "(k-?lined|d-?lined|ircd?\.|error|reset|timeout|closed|peer|\.net|timed|eof|lost)" $text] {
		...
		bMotion_metakit_update bmotion quits nick $nick waserror 1 
		return 0
	}

	bMotion_metakit_update bmotion quits nick $nick waserror 0
...
if {[bMotion_has_metakit]} {
	mk::view layout bmotion.quits "nick waserror"
	bMotion_putloglev 1 * "created metakit view for quits"
}
